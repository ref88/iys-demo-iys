...
const ShiftAssignmentModal = ({
  ...
  const handleSwapApproval = (requestId, status) => {
    const updatedRequests = swapRequests.map((req) =>
      req.id === requestId
        ? { ...req, status, approvedBy: currentUser?.id }
        : req
    );
      ...
      <div className='bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto'>
          ...
          <div className='bg-gray-50 rounded-lg p-4'>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
              ...
              <div className='flex items-center gap-2'>
                ...
                <span className='text-sm font-medium'>
                  Tijd: {shift.startTime} - {shift.endTime}
                </span>
              </div>
              <div className='flex items-center gap-2'>
                <Users className='w-5 h-5 text-gray-500' />
                <span className='text-sm font-medium'>
                  Max: {shiftType?.maxStaff} medewerkers
                </span>
              </div>
            </div>
          </div>
                        ...
                        <div className='flex items-center gap-2'>
                          ...
                          {canManageSwaps && request.status === 'pending' && (
                            <div className='flex gap-1'>
                              <button
                                onClick={() =>
                                  handleSwapApproval(request.id, 'approved')
                                }
                              ...
                              <button
                                onClick={() =>
                                  handleSwapApproval(request.id, 'rejected')
                                }
              ...
              <div className='space-y-3'>
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setSwapRequest({
                        ...swapRequest,
                        requesterId: parseInt(e.target.value),
                      })
                    }
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setSwapRequest({ ...swapRequest, reason: e.target.value })
                    }
                    className='w-full border rounded-lg px-3 py-2'
                    rows='3'
        ...
        <div className='flex items-center justify-between p-6 border-t'>
          <div className='text-sm text-gray-600'>
            {selectedStaff.length} van {shiftType?.maxStaff} medewerkers
            toegewezen
          </div>
          <div className='flex gap-2'>
            <button
