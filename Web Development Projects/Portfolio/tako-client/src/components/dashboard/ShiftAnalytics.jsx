...
const ShiftAnalytics = ({
  ...
  const calculateAnalytics = () => {
      ...
      shift.assignedStaff.forEach((staffId) => {
        ...
        staff.productivityScore = Math.round(
          ((staff.onTimeShifts / staff.totalShifts) * 0.4 +
            ...
            (1 - staff.noShows / staff.totalShifts) * 0.3) *
            100
        );
      });
    ...
    analytics.lateRate =
      totalCheckIns > 0 ? (lateCheckIns / totalCheckIns) * 100 : 0;
    analytics.noShowRate =
      totalCheckIns > 0 ? (noShows / totalCheckIns) * 100 : 0;
    analytics.averageShiftsPerStaff =
      staffMembers.length > 0 ? analytics.totalShifts / staffMembers.length : 0;

    // Calculate overall productivity score
    const avgProductivity =
      Object.values(analytics.staffPerformance).reduce(
        (sum, staff) => sum + staff.productivityScore,
        0
      ) / Object.keys(analytics.staffPerformance).length;
    ...
    analytics.costAnalysis.averageCostPerShift =
      analytics.totalShifts > 0
        ? analytics.costAnalysis.totalCost / analytics.totalShifts
        : 0;
    analytics.costAnalysis.costPerHour =
      analytics.totalHours > 0
        ? analytics.costAnalysis.totalCost / analytics.totalHours
        : 0;
  ...
  const exportAnalytics = () => {
    ...
    if (exportFormat === 'json') {
      ...
      const csvRows = [
        ...
        ...Object.entries(analytics.staffPerformance).map(
          ([staffId, performance]) => {
    ...
    <div className='space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-gray-600'>
            Performance insights en KPI's voor optimale dienstplanning
          </p>
        </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
        <div className='bg-white rounded-xl p-6 shadow-sm border border-gray-200'>
          ...
          <div className='mt-4'>
            <div className='flex items-center text-sm text-gray-600'>
              ...
              <span>
                {analytics.averageShiftsPerStaff.toFixed(1)} gemiddeld per
                personeel
              </span>
            </div>
          </div>
        ...
        <div className='bg-white rounded-xl p-6 shadow-sm border border-gray-200'>
          ...
          <div className='mt-4'>
            <div className='flex items-center text-sm text-gray-600'>
              ...
              <span>
                €{analytics.costAnalysis.totalCost.toFixed(0)} totale kosten
              </span>
            </div>
          </div>
        </div>
        ...
        <div className='bg-white rounded-xl p-6 shadow-sm border border-gray-200'>
          <div className='flex items-center justify-between'>
            <div>
              <p className='text-sm font-medium text-gray-600'>
                Productiviteit Score
              </p>
              <p
                className={`text-2xl font-bold ${getPerformanceColor(analytics.productivityScore)}`}
              >
        ...
        <div className='bg-white rounded-xl p-6 shadow-sm border border-gray-200'>
          ...
          <div className='mt-4'>
            <div className='flex items-center text-sm text-gray-600'>
              ...
              <span>
                {analytics.lateRate.toFixed(1)}% te laat,{' '}
                {analytics.noShowRate.toFixed(1)}% afwezig
              </span>
            </div>
          </div>
        </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
                    ...
                    <div className='flex items-center gap-3'>
                      ...
                      <div>
                        ...
                        <p className='text-sm text-gray-600'>
                          {performance.totalShifts} diensten
                        </p>
                      </div>
                    </div>
                    <div className='text-right'>
                      <p
                        className={`font-bold ${getPerformanceColor(performance.productivityScore)}`}
                      >
                        {performance.productivityScore}%
                      </p>
                      <p className='text-xs text-gray-600'>
                        {performance.totalHours}u
                      </p>
                    </div>
                  </div>
                );
        ...
        <div className='bg-white rounded-xl p-6 shadow-sm border border-gray-200'>
          <h3 className='text-lg font-semibold text-gray-900 mb-4'>
            Dienst Type Verdeling
          </h3>
          <div className='space-y-3'>
            {Object.entries(analytics.shiftTypeDistribution).map(
              ([shiftType, count]) => {
                ...
                return (
                  ...
                  >
                    <div className='flex items-center gap-3'>
                      <div
                        className={`w-4 h-4 rounded-full ${shiftInfo.color}`}
                      />
      ...
      <div className='bg-white rounded-xl p-6 shadow-sm border border-gray-200'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
          <div className='text-center'>
            <p className='text-2xl font-bold text-gray-900'>
              €{analytics.costAnalysis.totalCost.toFixed(0)}
            </p>
          ...
          <div className='text-center'>
            <p className='text-2xl font-bold text-gray-900'>
              €{analytics.costAnalysis.averageCostPerShift.toFixed(0)}
            </p>
            <p className='text-sm text-gray-600'>
              Gemiddelde kosten per dienst
            </p>
          </div>

          <div className='text-center'>
            <p className='text-2xl font-bold text-gray-900'>
              €{analytics.costAnalysis.costPerHour.toFixed(0)}
            </p>
