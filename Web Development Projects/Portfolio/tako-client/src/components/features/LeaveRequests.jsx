...
const LeaveRequests = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // Load leave requests
        let savedRequests = await DataService.getLeaveRequestsAsync();
        ...
        const savedResidents = await DataService.getResidentsAsync();
        const locationResidents = (savedResidents || []).filter(
        ...
        notify(
          'Er is een fout opgetreden bij het laden van de verlofaanvragen',
          { type: 'error' }
        );
      } finally {
        setIsLoading(false);
  ...
  useEffect(() => {
    ...
    filtered = filtered.filter(
      (request) => request.locationType === locationType
    );
  ...
  const handleAddRequest = (newRequest) => {
    try {
      ...
      // Add notification
      notify(`${newRequest.residentName} heeft een verlofaanvraag ingediend`, {
        type: 'success',
        persistent: true,
      ...
      notify(
        'Er is een fout opgetreden bij het toevoegen van de verlofaanvraag',
        { type: 'error' }
      );
    }
  };
  ...
  const handleApproveRequest = (requestId) => {
    try {
      ...
      // Add notification
      notify(`Verlofaanvraag van ${request.residentName} goedgekeurd`, {
        type: 'success',
        persistent: true,
        action: 'view_leave_request',
      ...
      notify(
        'Er is een fout opgetreden bij het goedkeuren van de verlofaanvraag',
        { type: 'error' }
      );
    }
  };
  ...
  const handleRejectRequest = (requestId, reason) => {
    try {
      ...
      notify({
        ...
        message:
          'Er is een fout opgetreden bij het afwijzen van de verlofaanvraag.',
      });
    }
  };
    ...
    if (
      ...
      try {
        ...
        notify({
          ...
          message:
            'Er is een fout opgetreden bij het verwijderen van de verlofaanvraag.',
        });
      }
    }
  };
      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4'>
        <div>
          ...
          <p className='text-gray-600 mt-1'>
            Beheer alle verlofaanvragen ({filteredRequests.length} weergegeven
            van {requests.length} totaal)
          </p>
        </div>
        <button
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-2xl font-bold text-yellow-600'>
                {
                  filteredRequests.filter((r) => r.status === 'In behandeling')
                    .length
                }
              </p>
            </div>
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-2xl font-bold text-green-600'>
                {
                  filteredRequests.filter((r) => r.status === 'Goedgekeurd')
                    .length
                }
              </p>
            </div>
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-2xl font-bold text-red-600'>
                {
                  filteredRequests.filter((r) => r.status === 'Afgewezen')
                    .length
                }
              </p>
            </div>
          ...
          <div className='space-y-4'>
            <div className='bg-gray-50 p-4 rounded-lg'>
              ...
              <div className='space-y-2 text-sm'>
                <div>
                  <span className='font-medium'>Bewoner:</span>{' '}
            ...
            <div className='flex justify-end gap-3 pt-4 border-t'>
              ...
              <Button
                variant={
                  selectedRequest.action === 'approve' ? 'success' : 'danger'
                }
                onClick={handleApprovalSubmit}
                disabled={
                  selectedRequest.action === 'reject' && !approvalReason.trim()
                }
                icon={
                  selectedRequest.action === 'approve' ? CheckCircle : XCircle
                }
              >
                {selectedRequest.action === 'approve'
                  ? 'Goedkeuren'
