// 📊 Excel Export Utilities for TAKO VMS Analytics
...
export const exportCapacityData = (residents, locationType) => {
  const locationResidents = residents.filter(
    (r) => r.locationType === locationType
  );

  // Calculate key metrics
  const capacity = locationType === 'CNO' ? 120 : 80;
  const present = locationResidents.filter(
    (r) => r.attendance === 'Aanwezig'
  ).length;
  const onLeave = locationResidents.filter(
    (r) => r.attendance === 'Op verlof'
  ).length;
  const absent = locationResidents.filter(
  ...
  const csvContent = [
    ...
    [
      'Beschikbaar',
      capacity - present,
      `${(((capacity - present) / capacity) * 100).toFixed(1)}%`,
    ],
    [''],
...
export const exportAgeGroupData = (residents, locationType) => {
  ...
  const csvContent = [
    // Header
    ['TAKO VMS - Leeftijdsgroepen Analyse'],
    ['Locatie:', locationType === 'CNO' ? 'CNO' : 'Oekraïne'],
    ['Gegenereerd op:', new Date().toLocaleString('nl-NL')],
    [''],
...
export const exportSchoolData = (residents, locationType) => {
  ...
  const csvContent = [
    ...
    [
      'Op school',
      schoolStats.attending,
      `${schoolStats.totalMinors > 0 ? ((schoolStats.attending / schoolStats.totalMinors) * 100).toFixed(1) : 0}%`,
    ],
    [
      'Geen school',
      schoolStats.notAttending,
      `${schoolStats.totalMinors > 0 ? ((schoolStats.notAttending / schoolStats.totalMinors) * 100).toFixed(1) : 0}%`,
    ],
    [
      'Leerplichtige leeftijd',
      schoolStats.compulsoryAge,
      `${schoolStats.totalMinors > 0 ? ((schoolStats.compulsoryAge / schoolStats.totalMinors) * 100).toFixed(1) : 0}%`,
    ],
    [''],
...
export const exportCombinedReport = (residents, locationType) => {
  ...
  const csvContent = [
    // Header
    ['TAKO VMS - Volledig Analytics Rapport'],
    ['Locatie:', locationType === 'CNO' ? 'CNO' : 'Oekraïne'],
    ['Gegenereerd op:', new Date().toLocaleString('nl-NL')],
    [''],
...
export const exportMonthlyTrendsData = (
  residents,
  locationType,
  monthlyData
) => {
  const csvContent = [
    ...
    [
      ...
      Math.round(
        monthlyData.reduce((sum, m) => sum + m.occupancy, 0) /
          monthlyData.length
      ),
    ],
    [
      'Totale instroom (12 maanden):',
      monthlyData.reduce((sum, m) => sum + m.instroom, 0),
    ],
    [
      'Totale uitstroom (12 maanden):',
      monthlyData.reduce((sum, m) => sum + m.uitstroom, 0),
    ],
    [
      'Netto groei (12 maanden):',
      monthlyData.reduce((sum, m) => sum + m.netto, 0),
    ],
...
export const exportAnalytics = {
  capacity: (residents, locationType) => {
    ...
    const filename = `TAKO_Capaciteit_${locationType}_${new Date().toISOString().split('T')[0]}.csv`;
    downloadCSV(csvContent, filename);
  },

  ageGroups: (residents, locationType) => {
    const csvContent = exportAgeGroupData(residents, locationType);
    const filename = `TAKO_Leeftijdsgroepen_${locationType}_${new Date().toISOString().split('T')[0]}.csv`;
    downloadCSV(csvContent, filename);
  },

  school: (residents, locationType) => {
    const csvContent = exportSchoolData(residents, locationType);
    const filename = `TAKO_School_${locationType}_${new Date().toISOString().split('T')[0]}.csv`;
    downloadCSV(csvContent, filename);
  },

  monthlyTrends: (residents, locationType, monthlyData) => {
    const csvContent = exportMonthlyTrendsData(
      residents,
      locationType,
      monthlyData
    );
    const filename = `TAKO_Maandelijks_Trends_${locationType}_${new Date().toISOString().split('T')[0]}.csv`;
    downloadCSV(csvContent, filename);
  },

  combined: (residents, locationType) => {
    const csvContent = exportCombinedReport(residents, locationType);
    const filename = `TAKO_Volledig_Rapport_${locationType}_${new Date().toISOString().split('T')[0]}.csv`;
    downloadCSV(csvContent, filename);
  },
};
