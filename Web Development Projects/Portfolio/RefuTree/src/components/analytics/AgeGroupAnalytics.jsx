...
import { 
  Users, Baby, GraduationCap, User, UserCheck, Calendar, 
  BarChart3, PieChart, TrendingUp, Download, Filter, 
  ChevronDown, ChevronUp, Eye, EyeOff, Target
} from 'lucide-react';
  ...
  >
    ...
    {isExpanded && onClick && (
      <div className="mt-4 pt-4 border-t border-gray-200">
        {onClick ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
      </div>
    )}
  </div>
);
...
const AgeDistributionChart = ({ ageGroups, totalResidents }) => (
  <div className="bg-white rounded-xl border border-gray-200 p-6">
    <div className="flex items-center justify-between mb-6">
      <h3 className="text-lg font-semibold text-gray-900">Leeftijdsverdeling Overzicht</h3>
      <div className="flex items-center space-x-2">
    ...
    <div className="space-y-4">
      {Object.entries(ageGroups).map(([group, data]) => {
        const percentage = totalResidents > 0 ? ((data.total / totalResidents) * 100).toFixed(1) : 0;
        return (
          <div key={group} className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="font-medium text-gray-700">{group} jaar</span>
              <span className="text-gray-600">{data.total} ({percentage}%)</span>
            </div>
...
const GenderDistribution = ({ residents }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-6">Geslachtsverdeling</h3>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="text-center">
          ...
          <div className="text-2xl font-bold text-gray-900">{genderStats.male}</div>
          <div className="text-sm text-gray-500">Man ({genderStats.malePercentage}%)</div>
        </div>
        ...
        <div className="text-center">
          ...
          <div className="text-2xl font-bold text-gray-900">{genderStats.female}</div>
          <div className="text-sm text-gray-500">Vrouw ({genderStats.femalePercentage}%)</div>
        </div>
      </div>
...
const AgeStatsSummary = ({ residents }) => {
  const ageStats = useMemo(() => {
    const ages = residents.map(r => calculateAge(r.birthDate)).filter(age => age !== null);
    ...
    const avg = Math.round(ages.reduce((sum, age) => sum + age, 0) / ages.length);
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-6">Leeftijd Statistieken</h3>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="text-center">
          <div className="text-2xl font-bold text-green-600">{ageStats.max}</div>
          <div className="text-sm text-gray-500">Oudste</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-purple-600">{ageStats.avg}</div>
          <div className="text-sm text-gray-500">Gemiddeld</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-orange-600">{ageStats.median}</div>
    ...
    <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500">Totaal Bewoners</p>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {ageGroups['0-4'].total + ageGroups['4-12'].total + ageGroups['12-18'].total}
              </p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{ageGroups['18-64'].total}</p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">{ageGroups['65+'].total}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
          {ageGroupConfig.map((config) => {
            ...
            const percentage = totalResidents > 0 ? 
              ((groupData.total / totalResidents) * 100).toFixed(1) : 0;
