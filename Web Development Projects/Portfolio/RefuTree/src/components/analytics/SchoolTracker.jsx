...
import { 
  GraduationCap, BookOpen, Users, AlertTriangle, CheckCircle, 
  XCircle, Clock, Target, TrendingUp, Download, Search, Filter,
  School, Baby, UserCheck, Calendar, Edit, Eye, Plus
} from 'lucide-react';
...
const SchoolStatusBadge = ({ status }) => {
  const statusConfig = {
    ...
    'NIET_LEERPLICHTIG': { color: 'bg-gray-100 text-gray-800', label: 'Niet leerplichtig' },
    'LEERPLICHTIG': { color: 'bg-orange-100 text-orange-800', label: 'Leerplichtig' }
  };
...
const SchoolTypeBadge = ({ type }) => {
  const typeConfig = {
    ...
    'MIDDELBARE': { color: 'bg-purple-100 text-purple-800', label: 'Middelbare school' },
    'ISK': { color: 'bg-yellow-100 text-yellow-800', label: 'ISK' },
    'SPECIAAL': { color: 'bg-pink-100 text-pink-800', label: 'Speciaal onderwijs' }
  };

  const config = typeConfig[type] || { color: 'bg-gray-100 text-gray-800', label: 'Onbekend' };
...
const SchoolStatsCard = ({ title, value, subtitle, icon: Icon, color = 'blue', percentage }) => (
...
const SchoolDistributionChart = ({ schoolStats }) => (
  <div className="bg-white rounded-xl border border-gray-200 p-6">
    <h3 className="text-lg font-semibold text-gray-900 mb-6">Onderwijs Verdeling</h3>
    
    <div className="space-y-4">
      {Object.entries(schoolStats.schoolTypes).map(([type, count]) => {
        const percentage = schoolStats.totalMinors > 0 ? 
          ((count / schoolStats.totalMinors) * 100).toFixed(1) : 0;
    ...
    <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <SchoolStatsCard
          ...
          icon={CheckCircle}
          color="green"
          percentage={schoolStats.totalMinors > 0 ? 
            ((schoolStats.attending / schoolStats.totalMinors) * 100).toFixed(1) : 0}
        />
        <SchoolStatsCard
          title="Geen School"
          ...
          color="red"
          percentage={schoolStats.totalMinors > 0 ? 
            ((schoolStats.notAttending / schoolStats.totalMinors) * 100).toFixed(1) : 0}
        />
        <SchoolStatsCard
          ...
          color="orange"
          percentage={schoolStats.totalMinors > 0 ? 
            ((schoolStats.compulsoryAge / schoolStats.totalMinors) * 100).toFixed(1) : 0}
        />
      </div>
          ...
          <div className="space-y-3">
            {schoolStats.compulsoryAge > schoolStats.attending && (
              <div className="flex items-center p-3 bg-orange-50 border border-orange-200 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-orange-800">Leerplichtige kinderen zonder school</p>
                  <p className="text-sm text-orange-700">
                    {schoolStats.compulsoryAge - schoolStats.attending} kinderen van leerplicht leeftijd volgen geen onderwijs
                  </p>
                </div>
              </div>
            )}
            ...
            {schoolStats.notAttending > 0 && (
              <div className="flex items-center p-3 bg-red-50 border border-red-200 rounded-lg">
                ...
                <div>
                  <p className="text-sm font-medium text-red-800">Kinderen zonder onderwijs</p>
                  <p className="text-sm text-red-700">
                    {schoolStats.notAttending} kinderen volgen momenteel geen onderwijs
                  </p>
                </div>
              </div>
            )}
            
            {schoolStats.attending === schoolStats.totalMinors && schoolStats.totalMinors > 0 && (
              <div className="flex items-center p-3 bg-green-50 border border-green-200 rounded-lg">
                <CheckCircle className="w-5 h-5 text-green-600 mr-3" />
                <div>
                  <p className="text-sm font-medium text-green-800">Alle kinderen op school</p>
