...
import { Tag, X, Plus, Check, ChevronDown } from 'lucide-react';
import LabelChip from '../ui/LabelChip.jsx';

const LabelSelector = ({ selectedLabels = [], onLabelsChange, availableLabels = [] }) => {
    ...
    <div className="relative">
      ...
      >
        <div className="flex flex-wrap gap-1">
          {selectedLabels.length === 0 && <span className="text-gray-400 text-sm">Geen labels</span>}
        ...
        <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
          <div className="p-2">
            ...
            {filteredLabels.length === 0 && (
              <div className="text-gray-400 text-sm px-2 py-1">Geen labels gevonden</div>
            )}
            {filteredLabels.map(label => (
              ...
              >
                ...
                {selectedLabels.includes(label.id) && <Check className="w-4 h-4 text-green-500" />}
              </button>
            ))}
          </div>
        </div>
