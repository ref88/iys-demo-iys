...
import { 
  Download, Upload, FileText, Users, Calendar, 
  CheckCircle, AlertTriangle, Info, Database,
  RefreshCw, Trash2, Save, Loader, Eye, 
  BarChart, Shield, Clock, PieChart
} from 'lucide-react';
...
const DataExport = () => {
  ...
  const generatePDFReport = async (reportType) => {
    try {
      ...
      doc.text(`Gegenereerd op: ${new Date().toLocaleDateString('nl-NL')}`, 10, 32);
      doc.text(`Door: ${currentUser?.name || 'Onbekend'}`, 10, 39);
      doc.text(`Periode: ${new Date(dateRange.start).toLocaleDateString('nl-NL')} - ${new Date(dateRange.end).toLocaleDateString('nl-NL')}`, 10, 46);
      ...
      switch (reportType) {
        ...
        case 'incidents':
          ...
          y = await generateIncidentRedirectNote(doc, y);
          break;
        case 'handovers':
      ...
      for (let i = 1; i <= pageCount; i++) {
        doc.setPage(i);
        doc.setFontSize(8);
        doc.text(`Pagina ${i} van ${pageCount} - RefuTree VMS Management Rapport`, 10, 287);
  ...
  const generateResidentsReport = async (doc, startY) => {
    ...
    y += 8;
    doc.setFontSize(10);
    doc.text(`Totaal bewoners: ${totalResidents}`, 15, y); y += 6;
    doc.text(`Aanwezig: ${presentResidents}`, 15, y); y += 6;
    doc.text(`Op verlof: ${onLeave}`, 15, y); y += 6;
    doc.text(`Afwezig: ${absent}`, 15, y); y += 6;
    doc.text(`Hoge prioriteit: ${highPriority}`, 15, y); y += 10;
    ...
    if (highPriority > 0) {
      ...
      residents.filter(r => r.priority === 'High').forEach(resident => {
        doc.text(`• ${resident.name} (${resident.nationality}) - ${resident.status}`, 15, y);
        y += 6;
        if (resident.notes) {
          const noteLines = doc.splitTextToSize(`  Notities: ${resident.notes}`, 170);
  ...
  const generateIncidentsReport = async (doc, startY) => {
    ...
    // Section header
    doc.setFontSize(14);
    doc.text('INCIDENTEN RAPPORT', 10, y);
    y += 10;
    ...
    const highIncidents = filteredIncidents.filter(i => i.priority === 'high').length;
    
    doc.text('STATISTIEKEN', 10, y);
    y += 8;
    doc.setFontSize(10);
    doc.text(`Totaal incidenten: ${totalIncidents}`, 15, y); y += 6;
    doc.text(`Open: ${openIncidents}`, 15, y); y += 6;
    doc.text(`Opgelost: ${resolvedIncidents}`, 15, y); y += 6;
    doc.text(`Kritieke prioriteit: ${criticalIncidents}`, 15, y); y += 6;
    doc.text(`Hoge prioriteit: ${highIncidents}`, 15, y); y += 10;
    ...
    doc.setFontSize(12);
    doc.text('INCIDENT TYPES', 10, y);
    y += 8;
    doc.setFontSize(10);
    Object.entries(typeStats).forEach(([type, count]) => {
      doc.text(`${type}: ${count}`, 15, y);
      y += 6;
    });
    y += 10;
    
    // Recent critical/high incidents
    const criticalHighIncidents = filteredIncidents.filter(i => 
      i.priority === 'critical' || i.priority === 'high'
    ).slice(0, 10);
    
    if (criticalHighIncidents.length > 0) {
      if (y > 230) {
        doc.addPage();
        y = 20;
      }
      ...
      doc.setFontSize(12);
      doc.text('KRITIEKE EN HOGE PRIORITEIT INCIDENTEN', 10, y);
      y += 8;
      doc.setFontSize(10);
      
      criticalHighIncidents.forEach(incident => {
        doc.text(`• ${incident.title} (${incident.priority})`, 15, y);
        y += 5;
        doc.text(`  ${new Date(incident.reportedAt).toLocaleDateString('nl-NL')} - ${incident.type}`, 20, y);
        y += 5;
        if (incident.description) {
          const descLines = doc.splitTextToSize(`  ${incident.description.substring(0, 100)}...`, 170);
  ...
  const generateHandoversReport = async (doc, startY) => {
    ...
    const filteredHandovers = handovers.filter(handover => {
      ...
      return handoverDate >= new Date(dateRange.start) && handoverDate <= new Date(dateRange.end);
    });
    ...
    const completedHandovers = filteredHandovers.filter(h => h.completed).length;
    ...
    y += 8;
    doc.setFontSize(10);
    doc.text(`Totaal overdrachten: ${totalHandovers}`, 15, y); y += 6;
    doc.text(`Afgeronde overdrachten: ${completedHandovers}`, 15, y); y += 6;
    doc.text(`Gemiddelde volledigheid: ${avgProgress}%`, 15, y); y += 10;
    ...
    if (recentHandovers.length > 0) {
      ...
      recentHandovers.forEach(handover => {
        ...
        y += 5;
        doc.text(`  ${new Date(handover.datum).toLocaleDateString('nl-NL')} (${handover.completed ? 'Afgerond' : 'In behandeling'})`, 20, y);
        y += 5;
        if (handover.sfeerimpressie) {
          const moodLines = doc.splitTextToSize(`  Sfeer: ${handover.sfeerimpressie.substring(0, 80)}...`, 170);
  ...
  const generateAnalyticsReport = async (doc, startY) => {
    ...
    const occupancyRate = residents.filter(r => r.status === 'Aanwezig').length / Math.max(residents.length, 1) * 100;
    const incidentRate = incidents.length / Math.max(residents.length, 1);
    const handoverCompletionRate = handovers.filter(h => h.completed).length / Math.max(handovers.length, 1) * 100;
    
    doc.text(`Bezettingsgraad: ${occupancyRate.toFixed(1)}%`, 15, y); y += 6;
    doc.text(`Incidenten per bewoner: ${incidentRate.toFixed(2)}`, 15, y); y += 6;
    doc.text(`Overdracht afronding: ${handoverCompletionRate.toFixed(1)}%`, 15, y); y += 10;
    ...
    const highPriorityPercentage = residents.filter(r => r.priority === 'High').length / Math.max(residents.length, 1) * 100;
    const criticalIncidentPercentage = incidents.filter(i => i.priority === 'critical').length / Math.max(incidents.length, 1) * 100;
    
    doc.text(`• ${highPriorityPercentage.toFixed(1)}% van bewoners heeft hoge prioriteit`, 15, y); y += 6;
    doc.text(`• ${criticalIncidentPercentage.toFixed(1)}% van incidenten is kritiek`, 15, y); y += 6;
    
    if (incidents.length > 0) {
      const avgResponseTime = "< 24 uur"; // Simplified
      doc.text(`• Gemiddelde incident responstijd: ${avgResponseTime}`, 15, y); y += 6;
    }
    ...
    if (highPriorityPercentage > 30) {
      recommendations.push('Hoog percentage hoge prioriteit bewoners - extra aandacht vereist');
    }
    
    if (criticalIncidentPercentage > 20) {
      recommendations.push('Veel kritieke incidenten - preventieve maatregelen overwegen');
    }
    
    if (handoverCompletionRate < 80) {
      recommendations.push('Overdracht completion rate laag - training overwegen');
    }
    
    if (occupancyRate > 95) {
      recommendations.push('Zeer hoge bezetting - capaciteit uitbreiding overwegen');
    }
    
    if (recommendations.length === 0) {
      recommendations.push('Geen specifieke aanbevelingen - systeem functioneert goed');
    }
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600 text-sm">Download een backup van alle systeem data</p>
          </div>
          <button
            onClick={exportData}
        ...
        {exportStatus && (
          <div className={`p-3 rounded-md ${
            exportStatus.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200' :
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600 text-sm">Herstel data vanuit een backup bestand</p>
          </div>
        ...
        <div className="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <div className="flex items-start">
            ...
            <div>
              <p className="text-yellow-800 font-medium text-sm">Waarschuwing</p>
              <p className="text-yellow-700 text-sm">
                Het importeren van data zal alle huidige data vervangen. 
                Zorg ervoor dat je een backup hebt gemaakt voordat je importeert.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-gray-600 text-sm">Beheer en onderhoud van systeem data</p>
          </div>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Rapport Type</label>
            <select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <p className="text-sm text-gray-600 mb-3">Compleet overzicht van alle bewoners, statistieken en statusupdates.</p>
          <button
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <p className="text-sm text-gray-600 mb-3">Voor incident rapportage gebruik het Incident Management systeem.</p>
          <button
            disabled
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <p className="text-sm text-gray-600 mb-3">Overzicht van dienst overdrachten en volledigheid.</p>
          <button
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4">
          ...
          <p className="text-sm text-gray-600 mb-3">Trends, KPI's en management inzichten.</p>
          <button
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-1">Beheer data export, import en genereer management rapporten</p>
      </div>
