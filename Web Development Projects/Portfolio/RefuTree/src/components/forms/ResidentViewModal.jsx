...
import { 
  X, Phone, Mail, MapPin, Shield, Calendar, Clock, 
  User, FileText, AlertTriangle, Activity, ChevronDown, ChevronUp,
  Edit, Download, Star, CheckCircle, XCircle, Tag, Copy, ExternalLink,
  Heart, MessageCircle, UserCheck, Globe, Home, Briefcase, BookOpen,
  Heart as Hospital, AlertCircle, Settings, MoreVertical, Search, Filter,
  TrendingUp, BarChart3, PieChart, Target, Zap, Award, Flag,
  Eye, EyeOff, Maximize2, Minimize2, Printer, Share2, Camera,
  ChevronLeft, ChevronRight, RotateCcw, Save, Upload, Download as DownloadIcon
} from 'lucide-react';
...
// 📊 Info Card Component
const InfoCard = ({ icon: Icon, title, value, subtitle, color = 'blue', action }) => (
...
const DetailSection = ({ title, children, collapsible = false, defaultOpen = true }) => {
...
const ResidentViewModal = ({ resident, isOpen, onClose, onEdit, onLabelsUpdate, availableLabels = [] }) => {
  ...
  // 📊 Computed values
  const age = useMemo(() => calculateAge(resident?.birthDate), [resident?.birthDate]);
  const documentCount = useMemo(() => resident?.documents?.length || 0, [resident?.documents]);
  const labelCount = useMemo(() => selectedLabels?.length || 0, [selectedLabels]);

  // 🎨 Status colors
  const getStatusColor = (status) => {
    switch (status) {
      case 'Vergunninghouder': return 'green';
      case 'In procedure': return 'yellow';
      case 'Tijdelijke bescherming': return 'blue';
            ...
            <div className="flex items-center space-x-4">
              <div className="relative">
                ...
                <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full border-2 border-white ${
                  ...
                  resident.attendance === 'Op verlof' ? 'bg-blue-500' :
                  'bg-red-500'
                }`}></div>
              </div>
              <div>
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-600">{resident.room}</div>
              <div className="text-sm text-gray-500">Kamer</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">{resident.leaveBalance || 0}</div>
              <div className="text-sm text-gray-500">Verlof dagen</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">{documentCount}</div>
              <div className="text-sm text-gray-500">Documenten</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-orange-600">{labelCount}</div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <InfoCard
                  icon={CheckCircle}
                  title="Status"
                  value={<StatusBadge status={resident.status} color={getStatusColor(resident.status)} />}
                ...
                <InfoCard
                  ...
                  subtitle={resident.priority === 'High' ? 'Verhoogde aandacht vereist' : 'Standaard behandeling'}
              ...
              <DetailSection title="Persoonlijke informatie">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">Geboortedatum</label>
                    <p className="text-gray-900">{formatDate(resident.birthDate)}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">Leeftijd</label>
                    <p className="text-gray-900">{age || '-'} jaar</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">Geslacht</label>
                    <p className="text-gray-900">{resident.gender === 'M' ? 'Man' : 'Vrouw'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">Taal</label>
              ...
              <DetailSection title="Begeleiding">
                <div className="space-y-4 mt-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">Begeleider</label>
                    <p className="text-gray-900">{resident.caseworker || '-'}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">Aanwezigheid</label>
                    <StatusBadge status={resident.attendance} color={getAttendanceColor(resident.attendance)} />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">Laatst gezien</label>
                    <p className="text-gray-900">{resident.lastSeen || '-'}</p>
                  </div>
                  {resident.notes && (
                    <div>
                      <label className="block text-sm font-medium text-gray-500 mb-1">Opmerkingen</label>
                      <p className="text-gray-900 bg-gray-50 p-3 rounded-lg">{resident.notes}</p>
                    </div>
                  )}
                </div>
              </DetailSection>
                    ...
                    <div className="space-y-3">
                      {resident.documents.map((doc, index) => (
                        <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                          ...
                          <StatusBadge 
                            status={doc.status} 
                            color={doc.status === 'Geverifieerd' ? 'green' : 'yellow'} 
                          />
                        </div>
                      ))}
                    </div>
            ...
            <div className="space-y-6">
              <DetailSection title="Medische informatie">
                <div className="mt-4 space-y-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-2">Allergieën</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-2">Medicatie</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">Contactpersoon bij noodgeval</label>
                    <p className="text-gray-900">{resident.medicalInfo?.emergencyContact || '-'}</p>
                  </div>
                </div>
              </DetailSection>
            </div>
                ...
                <div className="mt-4">
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center space-x-3 text-sm">
                      ...
                      <span className="text-gray-600">Aanwezigheid gecontroleerd</span>
                      <span className="text-gray-400 ml-auto">2 uur geleden</span>
                    </div>
                    <div className="flex items-center space-x-3 text-sm">
                      <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
                      <span className="text-gray-600">Document geüpload</span>
                      <span className="text-gray-400 ml-auto">1 dag geleden</span>
                    </div>
                  </div>
                </div>
              </DetailSection>
